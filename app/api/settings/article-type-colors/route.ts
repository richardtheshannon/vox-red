...
export async function POST(request: Request) {
  try {
    ...
    const { colors, lightColors } = await request.json()

    // Save both dark and light colors
    await Promise.all([
      prisma.settings.upsert({
        where: { key: DARK_SETTINGS_KEY },
        update: { value: JSON.stringify(colors) },
        create: {
          key: DARK_SETTINGS_KEY,
          value: JSON.stringify(colors)
        }
      }),
      prisma.settings.upsert({
        where: { key: LIGHT_SETTINGS_KEY },
        update: { value: JSON.stringify(lightColors) },
        create: {
          key: LIGHT_SETTINGS_KEY,
          value: JSON.stringify(lightColors)
        }
      })
    ])
    ...
    console.error('Error saving article type colors:', error)
